...
// vue2.0中的options合并到ctx中
export function applyOptions(instance: ComponentInternalInstance) {
  const options = resolveMergedOptions(instance)
  // data、methods中的this，即是instance.proxy对象
  ...
  // 初始化顺序：props->inject->method->data->computed->watch
  if (injectOptions) {
  ...
  // 调用options的data方法获取组件数据，reactive变成响应式数据，
  // 挂载到实例上
  if (dataOptions) {
  ...
  if (computedOptions) {
    for (const key in computedOptions) {
      ...
      // 对于于computed定义为函数和定义为对象(get、set方法)两种情况
      ...
      const set =
        !isFunction(opt) && isFunction(opt.set)
          ...
          : __DEV__
            ...
            : NOOP
      // 转换为composition api形式的computed，返回WritableComputedRef类型
      const c = computed({
        get,
        set
      })
  ...
  // watch选项生成watcher，实际调用composition api的watch生成副作用函数
  if (watchOptions) {
  ...
  // 注册生命周期函数钩子，此时会将组件内部实例注入到钩子函数中
  // 执行钩子函数时会恢复当前组件实例，执行完置空，见apiLifecycle.ts中injectHook
  // 所有在所有生命周期函数中都可以调用getCurrentInstance获取组件内部实例
  ...
  // 注册生命周期钩子函数到instance，bc(beforeCreated)，c(created)
  // 实际调用composition api的onMounted等函数
  ...
  // 兼容性处理，beforeDestroy和destroyed生命周期钩子函数，3.0已经用onBeforeUnmount、onUnmounted代替了这两个函数
  if (__COMPAT__) {
    if (
      beforeDestroy &&
  ...
  // applied from mixins
  // instance.render是setup函数返回的(setup如果返回一个函数就会被当做render函数)
  // 可以setup的优先级是比render函数高的
  ...
  // 兼容处理，vue2.0的过滤器在3.0中已移除，过滤器需要使用计算属性或者在config.globalProperties中注册全局方法实现
  if (
    __COMPAT__ &&
    filters &&
